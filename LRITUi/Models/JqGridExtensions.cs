...
namespace GridDemo.Models
{
    public static class JqGridExtensions
    {
        ...
        private static IQueryable<T> ListAddSearchQuery<T>(IQueryable<T> baseList, IEnumerable<string> searchQuery, IEnumerable<string> searchColumns)
        {
            //if ( (searchQuery==null) | (searchColumns == null)) return baseList;            

            const string strpredicateFormat = "{0}.ToString().Contains(\"{1}\")";
            var searchExpression = new System.Text.StringBuilder();
            string orPart = String.Empty;
            var column = searchColumns.ToList();
            var query = searchQuery.ToList();
            ...
            for (int i = 0; i < searchColumns.Count(); i++)
            {
              ...
              searchExpression.AppendFormat(strpredicateFormat, column[i], query[i]);
              orPart = " AND ";
            }
